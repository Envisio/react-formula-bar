...
var FormulaBar = exports["default"] = /*#__PURE__*/function (_Component) {
  function FormulaBar(_props) {
    ...
    _defineProperty(_this, "generateSuggestions", function (_ref10) {
      ...
      var insideFunc = false;
      if (valueTail.match(/[A-Za-z]+\d*$/g)) {
        var _valueTail$match$reve = valueTail.match(/[A-Za-z\d]+$/g).reverse();
        var _valueTail$match$reve2 = _slicedToArray(_valueTail$match$reve, 1);
        valueLast = _valueTail$match$reve2[0];
      } else if (valueTail.match(/[A-Za-z]+\d*\([^\)]*/g)) {
        // in the context of a function
        valueLast = valueTail.match(/[A-Za-z]+\d*\(/g).reverse()[0].replace("(", "");
        insideFunc = true;
      } else {
        var _valueTail$reverse = valueTail.reverse();
        var _valueTail$reverse2 = _slicedToArray(_valueTail$reverse, 1);
        valueLast = _valueTail$reverse2[0];
      }
      if (insideFunc) {
