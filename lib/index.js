...
function RegularBar() {
  ...
  return <div style={{
    ...
    fontFamily: 'Monaco'
      ...
      <input onChange={onChange} value={value} onKeyDown={onKeyDown} style={{
      borderStyle: 'none',
      outline: 'none',
      width: '100%',
      padding: 0,
      ...
      margin: 0
        ...
        var type = _ref6.type,
            items = _ref6.items;
        return <_react.Fragment key={(0, _lodash.join)(['suggestion', 'group', type], '-')}>
            <dt style={{
            fontWeight: (0, _lodash.eq)(groupIndex, (0, _lodash.first)(highlight)) ? 'bold' : undefined
          }}>
              {type}
            </dt>
            {(0, _lodash.map)(items, function (_ref7, itemIndex) {
            var title = _ref7.title,
                description = _ref7.description;
            return <dd key={(0, _lodash.join)(['suggestion', 'group', type, 'item', title], '-')} style={{
              marginLeft: 0
            }} onClick={(0, _lodash.partial)(onClick, groupIndex, itemIndex)} onMouseOver={(0, _lodash.partial)(onMouseOver, groupIndex, itemIndex)}>
                <label style={{
