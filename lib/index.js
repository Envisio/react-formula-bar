  ...
  function FormulaBar(_props) {
    ...
    _defineProperty(_this, "generateSuggestions", function (_ref10) {
      ...
      if (valueTail.match(/[A-Za-z]+\d*$/g) && valueLastMatch1) {
        var _valueLastMatch1$reve3 = valueLastMatch1.reverse();
        var _valueLastMatch1$reve4 = _slicedToArray(_valueLastMatch1$reve3, 1);
        valueLast = _valueLastMatch1$reve4[0];
      } else if (valueTail.match(/[A-Za-z]+\d*\([^\)]*/g) && valueTail.match(/[A-Za-z]+\d*\(/g)) {
        // in the context of a function
        valueLast = valueTail.match(/[A-Za-z]+\d*\(/g).reverse()[0].replace("(", "");
        insideFunc = true;
      } else {
        var _Array$from$reverse3 = Array.from(valueTail).reverse();
        var _Array$from$reverse4 = _slicedToArray(_Array$from$reverse3, 1);
        valueLast = _Array$from$reverse4[0];
      }
      if (insideFunc) {
